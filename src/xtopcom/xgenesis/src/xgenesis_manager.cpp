...
void xtop_genesis_manager::init_genesis_block(std::error_code & ec) {
    ...
    {
        common::xaccount_address_t account(sys_contract_relay_table_block_addr1);
...
base::xauto_ptr<base::xvblock_t> xtop_genesis_manager::create_genesis_of_relay_account(base::xvaccount_t const & account, xenum_create_src_t src, std::error_code & ec) {
    ...
    data::xemptyblock_build_t bbuild(sys_contract_relay_table_block_addr1);
    bbuild.set_header_extra(data);
    xdbg("create_genesis_of_relay_account, %d, %s", data.size(), top::HexEncode(data).c_str());
