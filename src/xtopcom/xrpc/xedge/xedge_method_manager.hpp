...
void xedge_method_base<T>::sendTransaction_method(xjson_proc_t & json_proc, const std::string & ip) {
    ...
    if (m_archive_flag) {
        ...
        if (ret) {
            ...
            std::string err = std::string("transaction validation check error (") + std::to_string(ret) + ")";
        ...
        if (xverifier::xwhitelist_utl::check_whitelist_limit_tx(tx.get())) {
            ...
            throw xrpc_error{enum_xrpc_error_code::rpc_param_param_error, "whitelist check error"};
        }
        ...
        if (ret) {
            ...
            throw xrpc_error{enum_xrpc_error_code::rpc_param_param_error, "duration expiration check error"};
        }
