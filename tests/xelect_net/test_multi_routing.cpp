...
TEST_F(xtop_test_elect_net_fixture, TOP_4008_BENCH) {
    ...
    for (std::size_t index = 0; index < 10000; ++index) {
        ...
        std::thread t1 = std::thread(&create_new_routing_table, test_elect_manager, e, zid, static_cast<uint64_t>(index * 2));

        t2.join();
        t1.join();
        if (index >= 2) {
            common::xip2_t xip2_{net_id, zid, cid, gid, slot_id, node_size, 2 * (index - 2)};
